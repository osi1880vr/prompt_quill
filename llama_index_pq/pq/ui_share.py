...
class UiShare:
    ...
    def refresh_adetailer_checkpoints(self, chpt, number):
        ...
        return gr.update(choices=out_array, value=self.g.settings_data[f'automa_ad_checkpoint_{number}'])
    ...
    def check_variables(self, number):
        if f'automa_adetailer_enable_{number}' not in self.g.settings_data:
            self.g.settings_data[f'automa_adetailer_enable_{number}'] = False
            self.g.settings_data[f'automa_ad_checkpoint_{number}'] = 'None'
            self.g.settings_data[f'automa_ad_use_inpaint_width_height_{number}'] = False
            self.g.settings_data[f'automa_ad_model_{number}'] = 'face_yolov8m.pt'
    ...
    def generate_ad_block(self, number):
        ...
        with (gr.Row()):
            with gr.Column(scale=3):
                ...
                automa_ad_clip_skip = gr.Slider(1, 5, step=1, value=self.g.settings_data[f'automa_ad_clip_skip_{number}'],
                                                label="Clipskip",
                                                info="Clipskip 1-5.")

                automa_ad_confidence = gr.Slider(0, 1, step=0.1, value=self.g.settings_data[f'automa_ad_confidence_{number}'],
                                                 label="Confidence",
